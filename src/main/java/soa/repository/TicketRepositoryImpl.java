...
//    }
    ...
    public PagedTicketList findAll(Integer perPage, Integer curPage, String sortBy, String filterBy) {
        ...
        ArrayList<Order> orderList = getOrderList(sortBy, criteriaBuilder, from);
    ...
    private ArrayList<Ticket> findAll(String perPage, String curPage, CriteriaQuery<Ticket> select) {
        if (perPage != null && curPage != null) {
            ...
            return (ArrayList<Ticket>) typedQuery.getResultList();
        } else
            return findAll(select);
    }
    private ArrayList<Ticket> findAll(Integer pageSize, Integer pageNumber, CriteriaQuery<Ticket> select) {
    ...
    private ArrayList<Ticket> findAll(CriteriaQuery<Ticket> select) {
        TypedQuery<Ticket> typedQuery = em.createQuery(select);
        return (ArrayList<Ticket>) typedQuery.getResultList();
    }
    ...
    private ArrayList<Predicate> getPredicatesList(String filterBy, CriteriaBuilder criteriaBuilder, Root<Ticket> from) {
        ...
        if (filterBy != null) {
            ArrayList<String> notParsedFilters = new ArrayList<>(Arrays.asList(filterBy.split(";")));
            for (String filterString : notParsedFilters) {
                ArrayList<String> filter = new ArrayList<>(Arrays.asList(filterString.split(",")));
                switch (filter.get(0)) {
                    case ("id"):
...
//                        break;
    ...
    private ArrayList<Order> getOrderList(String sortBy, CriteriaBuilder criteriaBuilder, Root<Ticket> from) {
        ArrayList<Order> orderList = new ArrayList();
        if (sortBy != null) {
            ArrayList<String> criteria = new ArrayList<>(Arrays.asList(sortBy.split(";")));
